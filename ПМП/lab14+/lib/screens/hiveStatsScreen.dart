import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import '../bloc/appBLoC.dart';
import '../bloc/appEvent.dart';
import '../bloc/appState.dart';

class HiveStatsScreen extends StatefulWidget {
  const HiveStatsScreen({super.key});

  @override
  State<HiveStatsScreen> createState() => _HiveStatsScreenState();
}

class _HiveStatsScreenState extends State<HiveStatsScreen> {
  bool _isInitialized = false;

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      setState(() {
        _isInitialized = true;
      });
    });
  }

  Widget _buildStatItem(String label, int count, IconData icon) {
    return Column(
      children: [
        // Анимация вращения иконки
        TweenAnimationBuilder<double>(
          tween: Tween<double>(begin: 0.0, end: _isInitialized ? 1.0 : 0.0),
          duration: const Duration(milliseconds: 1000),
          curve: Curves.easeOutBack,
          builder: (context, rotationValue, child) {
            return Transform.rotate(
              angle: rotationValue * 2 * 3.14159, // Полный оборот
              child: Icon(icon, color: const Color(0xFFffae1a), size: 24),
            );
          },
        ),
        const SizedBox(height: 4),
        TweenAnimationBuilder<int>(
          tween: IntTween(begin: 0, end: _isInitialized ? count : 0),
          duration: const Duration(milliseconds: 1500),
          curve: Curves.easeOutCubic,
          builder: (context, animatedValue, child) {
            return Text(
              animatedValue.toString(),
              style: const TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.bold,
                color: Color(0xFF5d666f),
                fontFamily: 'Pretendard',
              ),
            );
          },
        ),
        Text(
          label,
          style: TextStyle(
            fontSize: 12,
            color: Colors.grey.shade500,
            fontFamily: 'Pretendard',
          ),
        ),
      ],
    );
  }

  void _showDemoResult(BuildContext context, String title, String message) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(title),
        content: Text(message),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('OK'),
          ),
        ],
      ),
    );
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Hive Statistics'),
        backgroundColor: Colors.white,
        elevation: 0,
        leading: IconButton(
          icon: const Icon(Icons.west_rounded),
          onPressed: () => Navigator.pop(context),
        ),
      ),
      body: BlocBuilder<AppBloc, AppState>(
        builder: (context, state) {
          if (state is AppLoading || (state is AppLoaded && state.isLoading)) {
            return const Center(
              child: CircularProgressIndicator(),
            );
          }

          if (state is! AppLoaded) {
            return const Center(
              child: Text('Ошибка загрузки данных'),
            );
          }

          final bloc = context.read<AppBloc>();
          final stats = bloc.getStats();

          return SingleChildScrollView(
            padding: const EdgeInsets.all(25),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // Статистика
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
                    color: Colors.grey.shade50,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: Colors.grey.shade200),
                  ),
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceAround,
                    children: [
                      _buildStatItem('Пользователи', stats['users'] ?? 0, Icons.people),
                      _buildStatItem('Книги', stats['books'] ?? 0, Icons.book),
                      _buildStatItem('Избранное', stats['favorites'] ?? 0, Icons.favorite),
                    ],
                  ),
                ),
                const SizedBox(height: 32),
                
                // Функции Hive
                const Text(
                  'Hive Functions',
                  style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                    fontFamily: 'Pretendard',
                    color: Color(0xFF5d666f),
                  ),
                ),
                const SizedBox(height: 16),
                Row(
                  children: [
                    Expanded(
                      child: ElevatedButton(
                        onPressed: () async {
                          final bloc = context.read<AppBloc>();
                          bloc.add(const EncryptionDemonstrated());
                          final result = await bloc.demonstrateEncryption();
                          _showDemoResult(context, 'Encryption Test', result);
                        },
                        style: ElevatedButton.styleFrom(
                          backgroundColor: const Color(0xFF6fbdbf),
                          foregroundColor: Colors.white,
                          padding: const EdgeInsets.symmetric(vertical: 12),
                        ),
                        child: const Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.security, size: 20),
                            SizedBox(width: 8),
                            Text(
                              'Test Encryption',
                              style: TextStyle(
                                fontFamily: 'Pretendard',
                                fontWeight: FontWeight.w600,
                              ),
                            ),
                          ],
                        ),
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: ElevatedButton(
                        onPressed: () async {
                          context.read<AppBloc>().add(const DataCompressed());
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('Data compressed successfully'),
                              backgroundColor: Colors.green,
                            ),
                          );
                        },
                        style: ElevatedButton.styleFrom(
                          backgroundColor: const Color(0xFF25b4c4),
                          foregroundColor: Colors.white,
                          padding: const EdgeInsets.symmetric(vertical: 12),
                        ),
                        child: const Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.compress, size: 20),
                            SizedBox(width: 8),
                            Text(
                              'Compress Data',
                              style: TextStyle(
                                fontFamily: 'Pretendard',
                                fontWeight: FontWeight.w600,
                              ),
                            ),
                          ],
                        ),
                      ),
                    ),
                  ],
                ),
                const SizedBox(height: 12),
                if (state.canManageBooks)
                  ElevatedButton(
                    onPressed: () {
                      Navigator.pushNamed(context, '/admin_books');
                    },
                    style: ElevatedButton.styleFrom(
                      backgroundColor: const Color(0xFFffae1a),
                      foregroundColor: Colors.white,
                      padding: const EdgeInsets.symmetric(vertical: 12),
                      minimumSize: const Size(double.infinity, 50),
                    ),
                    child: const Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.add, size: 20),
                        SizedBox(width: 8),
                        Text(
                          'Add New Book',
                          style: TextStyle(
                            fontFamily: 'Pretendard',
                            fontWeight: FontWeight.w600,
                          ),
                        ),
                      ],
                    ),
                  ),
              ],
            ),
          );
        },
      ),
    );
  }
}